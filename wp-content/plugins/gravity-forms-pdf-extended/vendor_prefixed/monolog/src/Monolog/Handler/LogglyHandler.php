...
use GFPDF_Vendor\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    public function __construct(string $token, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \GFPDF_Vendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \GFPDF_Vendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \GFPDF_Vendor\Monolog\Formatter\FormatterInterface
    {
        return new \GFPDF_Vendor\Monolog\Formatter\LogglyFormatter();
    }
}
