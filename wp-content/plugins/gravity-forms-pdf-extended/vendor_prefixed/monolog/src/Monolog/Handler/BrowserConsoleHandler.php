...
class BrowserConsoleHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \GFPDF_Vendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \GFPDF_Vendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \GFPDF_Vendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
