...
use GFPDF_Vendor\PhpConsole\Connector;
use GFPDF_Vendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use GFPDF_Vendor\PhpConsole\Helper;
/**
...
class PHPConsoleHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        if (!\class_exists('GFPDF_Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\GFPDF_Vendor\PhpConsole\Connector $connector = null) : \GFPDF_Vendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \GFPDF_Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \GFPDF_Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\GFPDF_Vendor\PhpConsole\Helper::isRegistered()) {
            \GFPDF_Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \GFPDF_Vendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \GFPDF_Vendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \GFPDF_Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \GFPDF_Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \GFPDF_Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
