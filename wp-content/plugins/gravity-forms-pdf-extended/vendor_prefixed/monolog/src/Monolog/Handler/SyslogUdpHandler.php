 ...
 */
class SyslogUdpHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractSyslogHandler
{
    const RFC3164 = 0;
    const RFC5424 = 1;
    const RFC5424e = 2;
    ...
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        ...
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \GFPDF_Vendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
     ...
     */
    public function setSocket(\GFPDF_Vendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
