...
use GFPDF_Vendor\Monolog\Formatter\FlowdockFormatter;
...
class FlowdockHandler extends \GFPDF_Vendor\Monolog\Handler\SocketHandler
{
    ...
    public function __construct(string $apiToken, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('openssl')) {
            throw new \GFPDF_Vendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FlowdockHandler');
        }
    ...
    {
        if (!$formatter instanceof \GFPDF_Vendor\Monolog\Formatter\FlowdockFormatter) {
    ...
    {
        return \GFPDF_Vendor\Monolog\Utils::jsonEncode($record['formatted']['flowdock']);
    }
    /**
