...
class FleepHookHandler extends \GFPDF_Vendor\Monolog\Handler\SocketHandler
{
    ...
    public function __construct(string $token, $level = \GFPDF_Vendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('openssl')) {
            throw new \GFPDF_Vendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FleepHookHandler');
        }
        $this->token = $token;
    ...
    {
        return new \GFPDF_Vendor\Monolog\Formatter\LineFormatter(null, null, \true, \true);
    }
    /**
