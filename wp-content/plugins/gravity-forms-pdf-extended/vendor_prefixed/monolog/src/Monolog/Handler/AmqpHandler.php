...
use GFPDF_Vendor\Monolog\Formatter\JsonFormatter;
...
class AmqpHandler extends \GFPDF_Vendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        if ($exchange instanceof \GFPDF_Vendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \GFPDF_Vendor\PhpAmqpLib\Message\AMQPMessage
    {
        return new \GFPDF_Vendor\PhpAmqpLib\Message\AMQPMessage($data, ['delivery_mode' => 2, 'content_type' => 'application/json']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \GFPDF_Vendor\Monolog\Formatter\JsonFormatter(\GFPDF_Vendor\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
