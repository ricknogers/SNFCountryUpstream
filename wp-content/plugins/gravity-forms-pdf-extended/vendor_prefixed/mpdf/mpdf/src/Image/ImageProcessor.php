...
use GFPDF_Vendor\Mpdf\RemoteContentFetcher;
    ...
    public function getImage(&$file, $firsttime = \true, $allowvector = \true, $orig_srcpath = \false, $interpolation = \false)
    {
        ...
        if (empty($data)) {
            ...
            if ($orig_srcpath && $this->mpdf->basepathIsLocal && ($check = @\fopen($orig_srcpath, 'rb'))) {
                \fclose($check);
                $file = $orig_srcpath;
                $this->logger->debug(\sprintf('Fetching (file_get_contents) content of file "%s" with local basepath', $file), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::REMOTE_CONTENT]);
                $data = \file_get_contents($file);
                $type = $this->guesser->guess($data);
            }
            if ($file && !$data && ($check = @\fopen($file, 'rb'))) {
                \fclose($check);
                $this->logger->debug(\sprintf('Fetching (file_get_contents) content of file "%s" with non-local basepath', $file), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::REMOTE_CONTENT]);
                $data = \file_get_contents($file);
                $type = $this->guesser->guess($data);
            }
