...
class Svg
{
    ...
    private $inDefs;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Otl $otl, \GFPDF_Vendor\Mpdf\CssManager $cssManager, \GFPDF_Vendor\Mpdf\Image\ImageProcessor $imageProcessor, \GFPDF_Vendor\Mpdf\SizeConverter $sizeConverter, \GFPDF_Vendor\Mpdf\Color\ColorConverter $colorConverter, \GFPDF_Vendor\Mpdf\Language\LanguageToFontInterface $languageToFont, \GFPDF_Vendor\Mpdf\Language\ScriptToLanguageInterface $scriptToLanguage)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
        ...
        if (isset($this->txt_data[2])) {
            ...
            if ($this->mpdf->useKerning) {
                if ($this->mpdf->CurrentFont['haskernGPOS']) {
                    ...
                    $textvar = $textvar | \GFPDF_Vendor\Mpdf\Css\TextVars::FC_KERNING;
                }
            }
    ...
    function markScriptToLang($html)
    {
        ...
        foreach ($a as $i => $e) {
            if ($i % 2 == 0) {
                $e = \GFPDF_Vendor\Mpdf\Utils\UtfString::strcode2utf($e);
                ...
                for ($sch = 0; $sch <= $subchunk; $sch++) {
                    if (isset($chardata[$sch])) {
                        ...
                        for ($j = 0; $j < \count($chardata[$sch]); $j++) {
                            $s .= \GFPDF_Vendor\Mpdf\Utils\UtfString::code2utf($chardata[$sch][$j]['uni']);
                        }
                        ...
                        if ($scriptblocks[$sch] == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_LATIN && $this->mpdf->autoVietnamese && \preg_match("/([" . $this->scriptToLanguage->getLanguageDelimiters('viet') . "])/u", $s)) {
                            $lang = "vi";
                        } elseif ($scriptblocks[$sch] == \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->mpdf->autoArabic) {
                            if (\preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('sindhi') . "]/u", $s)) {
                                ...
                                $lang = "fa";
                            } elseif ($this->mpdf->baseScript != \GFPDF_Vendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->scriptToLanguage->getLanguageByScript($scriptblocks[$sch])) {
    ...
    function xml_svg2pdf_start($parser, $name, $attribs)
    {
        ...
        switch (\strtolower($name)) {
            ...
            case 'svg':
                ...
                break;
            case 'path':
                $path = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($attribs, 'd', '');
