...
class Barcode
{
    ...
    public function getBarcode($code, $type, $pr = 0.0)
    {
        switch (\strtoupper($type)) {
            ...
            case 'EAN13':
                // EAN 13
                return new \GFPDF_Vendor\Mpdf\Barcode\EanUpc($code, 13, 11, 7, 0.33, 25.93);
            case 'UPCA':
                // UPC-A
                return new \GFPDF_Vendor\Mpdf\Barcode\EanUpc($code, 12, 9, 9, 0.33, 25.91);
            case 'UPCE':
                // UPC-E
                return new \GFPDF_Vendor\Mpdf\Barcode\EanUpc($code, 6, 9, 7, 0.33, 25.93);
            case 'EAN8':
                // EAN 8
                return new \GFPDF_Vendor\Mpdf\Barcode\EanUpc($code, 8, 7, 7, 0.33, 21.64);
            case 'EAN2':
                // 2-Digits UPC-Based Extention
                return new \GFPDF_Vendor\Mpdf\Barcode\EanExt($code, 2, 7, 7, 0.33, 20, 9);
            case 'EAN5':
                // 5-Digits UPC-Based Extention
                return new \GFPDF_Vendor\Mpdf\Barcode\EanExt($code, 5, 7, 7, 0.33, 20, 9);
            case 'IMB':
                ...
                $bpi = 22;
                // Bars per inch
                return new \GFPDF_Vendor\Mpdf\Barcode\Imb($code, $xdim, (25.4 / $bpi - $xdim) / $xdim, ['D' => 2, 'A' => 2, 'F' => 3, 'T' => 1]);
            case 'RM4SCC':
                ...
                $bpi = 22;
                // Bars per inch
                return new \GFPDF_Vendor\Mpdf\Barcode\Rm4Scc($code, $xdim, (25.4 / $bpi - $xdim) / $xdim, ['D' => 5, 'A' => 5, 'F' => 8, 'T' => 2]);
            case 'KIX':
                ...
                $bpi = 22;
                // Bars per inch
                return new \GFPDF_Vendor\Mpdf\Barcode\Rm4Scc($code, $xdim, (25.4 / $bpi - $xdim) / $xdim, ['D' => 5, 'A' => 5, 'F' => 8, 'T' => 2], \true);
            case 'POSTNET':
                // POSTNET
                $xdim = 0.508;
                // Nominal value for X-dim (bar width) in mm (spec.)
                $bpi = 22;
                // Bars per inch
                return new \GFPDF_Vendor\Mpdf\Barcode\Postnet($code, $xdim, (25.4 / $bpi - $xdim) / $xdim, \false);
            case 'PLANET':
                // PLANET
                $xdim = 0.508;
                // Nominal value for X-dim (bar width) in mm (spec.)
