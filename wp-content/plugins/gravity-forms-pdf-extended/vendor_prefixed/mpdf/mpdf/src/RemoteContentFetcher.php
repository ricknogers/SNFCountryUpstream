...
class RemoteContentFetcher implements \Psr\Log\LoggerAwareInterface
{
     ...
     */
    private $logger;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \Psr\Log\LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->logger = $logger;
    }
    ...
    {
        $this->logger->debug(\sprintf('Fetching (socket) content of remote URL "%s"', $url), ['context' => \GFPDF_Vendor\Mpdf\Log\Context::REMOTE_CONTENT]);
        // mPDF 5.7.3
        $timeout = 1;
        $p = \parse_url($url);
        $file = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($p, 'path', '');
        $scheme = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($p, 'scheme', '');
        $port = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($p, 'port', 80);
        $prefix = '';
        if ($scheme === 'https') {
            $prefix = 'ssl://';
            $port = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($p, 'port', 443);
        }
        $query = \GFPDF_Vendor\Mpdf\Utils\Arrays::get($p, 'query', null);
        if ($query) {
            $file .= '?' . $query;
        }
