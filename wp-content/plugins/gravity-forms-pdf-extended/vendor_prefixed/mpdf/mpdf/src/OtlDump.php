...
class OtlDump
{
    ...
    var $version;
    private $mpdf;
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf)
    {
        $this->mpdf = $mpdf;
    ...
    function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = \false, $BMPonly = \false, $kerninfo = \false, $useOTL = 0, $mode = null)
    {
        ...
        if (!\in_array($version, [0x10000, 0x74727565]) && !$TTCfontID) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Not a TrueType font: version=" . $version);
        }
        if ($TTCfontID > 0) {
            $this->version = $version = $this->read_ulong();
            // TTC Header version now
            if (!\in_array($version, [0x10000, 0x20000])) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error parsing TrueType Collection: version=" . $version . " - " . $file);
            }
    ...
    function extractInfo($debug = \false, $BMPonly = \false, $kerninfo = \false, $useOTL = 0)
    {
        ...
        if ($format != 0 && $format != 1) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error loading font: Unknown name table format " . $format);
        }
        ...
        for ($i = 0; $i < $numRecords; $i++) {
            ...
            if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) {
                ...
                if ($length % 2 != 0) {
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error loading font: PostScript name is UTF-16BE string of odd length");
                }
                $length /= 2;
                $N = '';
                while ($length > 0) {
        ...
        if ($debug) {
            for ($i = 0; $i < \count($psName); $i++) {
                ...
                if ($oc > 126 || \strpos(' [](){}<>/%', $c) !== \false) {
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException("psName=" . $psName . " contains invalid character " . $c . " ie U+" . \ord($c));
                }
            }
        }
        $this->name = $psName;
        ...
        if ($glyphDataFormat != 0) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Unknown glyph data format ' . $glyphDataFormat);
        }
        ///////////////////////////////////
        // hhea metrics table
        ///////////////////////////////////
        ...
        if (isset($this->tables["OS/2"])) {
            ...
            if ($fsType == 0x2 || ($fsType & 0x300) != 0) {
                ...
                if (!$overrideTTFFontRestriction) {
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Font file ' . $this->filename . ' cannot be embedded due to copyright restrictions.');
                }
        ...
        if ($debug) {
            ...
            if ($ver_maj < 1 || $ver_maj > 4) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Unknown post table version ' . $ver_maj);
            }
        } else {
            $this->skip(4);
        }
        ...
        if ($metricDataFormat != 0) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Unknown horizontal metric data format ' . $metricDataFormat);
        }
        ...
        if ($debug) {
            ...
            $ver_min = $this->read_ushort();
            if ($ver_maj != 1) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Unknown maxp table version ' . $ver_maj);
            }
        } else {
            $this->skip(4);
        }
        ...
        if (!$unicode_cmap_offset) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Font (' . $this->filename . ') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)');
        }
        $sipset = \false;
        $smpset = \false;
        // mPDF 5.7.1
...
?>';
                                ...
                                if ($Lookup[$i]['Type'] == 4) {
                                    ...
                                    if ($Lookup[$i]['Type'] == 5) {
                                        ...
                                        if ($Lookup[$i]['Type'] == 6) {
                                            ...
                                            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Lookup Type " . $Lookup[$i]['Type'] . " not supported.");
                                        }
                                    }
                                }
                            }
    ...
    function _getGSUBignoreString($flag, $MarkFilteringSet)
    {
        ...
        // Flag & 0x0010 = UseMarkFilteringSet
        if ($flag & 0x10) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("This font " . $this->fontkey . " contains MarkGlyphSets");
            $str = "Mark Glyph Set: ";
                                    ...
                                    */
                                    ...
                                    if ($Lookup[$luli]['Type'] == 6) {
                                        ...
                                        if ($Lookup[$luli]['Type'] == 7) {
                                            ...
                                            //===========
                                            if ($PosFormat == 1) {
                                                throw new \GFPDF_Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not YET TESTED.");
                                            } else {
                                                if ($PosFormat == 2) {
                                                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not YET TESTED.");
                                                } else {
                                                    if ($PosFormat == 3) {
                                                        ...
                                                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
                                                    }
                                                }
                                            }
                                        } else {
                                            if ($Lookup[$luli]['Type'] == 8) {
                                                ...
                                                //===========
                                                if ($PosFormat == 1) {
                                                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
                                                } else {
                                                    if ($PosFormat == 2) {
    ...
    function getGlyphs($originalGlyphIdx, &$start, &$glyphSet, &$subsetglyphs)
    {
        ...
        if ($numberOfContours < 0) {
            ...
            if ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::SCALE) {
                ...
                if ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::XYSCALE) {
                    $this->skip(4);
                } else {
                    if ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::TWOBYTWO) {
                        $this->skip(8);
                    }
                }
            }
