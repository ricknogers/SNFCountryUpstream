 ...
 */
class Msi extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param int $code
     * @param bool $checksum
    ...
    private function init($code, $checksum)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$digit])) {
                // invalid character
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in MSI barcode value "%s"', $digit, $code));
            }
            $seq .= $chr[$digit];
        }
        $seq .= '1001';
