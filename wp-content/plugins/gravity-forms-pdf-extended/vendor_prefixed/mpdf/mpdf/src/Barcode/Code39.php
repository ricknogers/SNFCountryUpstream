 ...
 */
class Code39 extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio, $extended, $checksum)
    {
        ...
        if ($code === \false) {
            throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid CODE39 barcode value "%s"', $code));
        }
        if ($checksum) {
            // checksum
