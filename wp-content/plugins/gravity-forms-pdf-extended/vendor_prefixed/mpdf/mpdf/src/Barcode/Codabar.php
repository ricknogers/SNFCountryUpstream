 ...
 */
class Codabar extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) {
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in CODABAR barcode value "%s"', $code[$i], $code));
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 8; ++$j) {
                if ($j % 2 == 0) {
