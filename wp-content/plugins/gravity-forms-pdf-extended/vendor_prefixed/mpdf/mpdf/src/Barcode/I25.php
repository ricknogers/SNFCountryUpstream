 ...
 */
class I25 extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio, $checksum)
    {
        ...
        for ($i = 0; $i < $clen; $i = $i + 2) {
            ...
            if (!isset($chr[$charBar]) or !isset($chr[$charSpace])) {
                // invalid character
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid I25 barcode value "%s"', $code));
            }
