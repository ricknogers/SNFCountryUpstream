 ...
 */
class Code93 extends \GFPDF_Vendor\Mpdf\Barcode\AbstractBarcode implements \GFPDF_Vendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
     */
    ...
    private function init($code)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            if (\ord($code[$i]) > 127) {
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid character "%s" in CODE93 barcode value "%s"', $code[$i], $code));
            }
        ...
        for ($i = 0; $i < $clen; ++$i) {
            ...
            if (!isset($chr[$char])) {
                // invalid character
                throw new \GFPDF_Vendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid CODE93 barcode value "%s"', $code));
            }
            for ($j = 0; $j < 6; ++$j) {
                if ($j % 2 == 0) {
                    $t = \true;
