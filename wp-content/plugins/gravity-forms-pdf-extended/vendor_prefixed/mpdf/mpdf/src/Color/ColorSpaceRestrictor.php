...
class ColorSpaceRestrictor
{
     ...
     * @param int $mode
     */
    public function __construct(\GFPDF_Vendor\Mpdf\Mpdf $mpdf, \GFPDF_Vendor\Mpdf\Color\ColorModeConverter $colorModeConverter, $mode)
    {
        $this->mpdf = $mpdf;
    ...
    private function restrictSpotColorSpace($c, &$PDFAXwarnings = [])
    {
        if (!isset($this->mpdf->spotColorIDs[$c[1]])) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Error: Spot colour has not been defined - ' . $this->mpdf->spotColorIDs[$c[1]]);
        }
        if ($this->mpdf->PDFA) {
