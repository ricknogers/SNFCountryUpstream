...
use GFPDF_Vendor\Mpdf\Fonts\GlyphOperator;
...
// mPDF 5.7.1
if (!\function_exists('GFPDF_Vendor\\Mpdf\\unicode_hex')) {
...
class TTFontFile
{
    ...
    public $GSUBLookups;
    public $GSLuCoverage;
    public function __construct(\GFPDF_Vendor\Mpdf\Fonts\FontCache $fontCache, $fontDescriptor)
    {
    ...
    public function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = \false, $BMPonly = \false, $useOTL = 0)
    {
        ...
        if (!$this->fh) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Unable to open font file "%s"', $file));
        }
        $this->_pos = 0;
        $this->charWidths = '';
        $this->glyphPos = [];
        ...
        if (!\in_array($version, [0x10000, 0x74727565], \true) && !$TTCfontID) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Not a TrueType font: version=%s)', $version));
        }
        if ($TTCfontID > 0) {
            $this->version = $version = $this->read_ulong();
            // TTC Header version now
            if (!\in_array($version, [0x10000, 0x20000], \true)) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error parsing TrueType Collection: version=%s - (%s)', $version, $file));
            }
        ...
        foreach ($this->tables as $t) {
            if ($t['length'] > 0 && $t['length'] < $this->maxStrLenRead) {
                ...
                if ($xchecksum != $checksum) {
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('TTF file "%s": invalid checksum %s table: %s (expected %s)', $this->filename, \dechex($checksum[0]) . \dechex($checksum[1]), $t['tag'], \dechex($xchecksum[0]) . \dechex($xchecksum[1])));
                }
            }
        }
    }
    ...
    function getCTG($file, $TTCfontID = 0, $debug = \false, $useOTL = \false)
    {
        ...
        $this->fh = \fopen($file, 'rb');
        if (!$this->fh) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Unable to open file "%s"', $file));
        }
        $this->_pos = 0;
        $this->charWidths = '';
        $this->glyphPos = [];
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff) {
                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Font "%s" cannot map all included glyphs into Private Use Area U+E000-U+F8FF; cannot use useOTL on this font', $file));
                    }
    ...
    function getTTCFonts($file)
    {
        ...
        if ($version === 0x74746366) {
            ...
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf("Not a TrueType Collection: version=%s (%s)", $version, $file));
        }
        $this->numTTCFonts = $this->read_ulong();
        for ($i = 1; $i <= $this->numTTCFonts; $i++) {
            $this->TTCFonts[$i]['offset'] = $this->read_ulong();
    ...
    function extractInfo($debug = \false, $BMPonly = \false, $useOTL = 0)
    {
        ...
        if ($format != 0 && $format != 1) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error loading font: Unknown name table format {$format} for font {$this->filename}");
        }
        ...
        for ($i = 0; $i < $numRecords; $i++) {
            ...
            if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) {
                ...
                if ($length % 2 != 0) {
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error loading font: PostScript name is UTF-16BE string of odd length for font {$this->filename}");
                }
                $length /= 2;
                $N = '';
                while ($length > 0) {
        ...
        if (!$psName) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Error loading font: Could not find PostScript font name '{$this->filename}'");
        }
        ...
        if ($debug) {
            ...
            if ($ver_maj != 1) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Unknown head table version ' . $ver_maj . '.' . $ver_min);
            }
            $this->fontRevision = $this->read_ushort() . $this->read_ushort();
            $this->skip(4);
            $magic = $this->read_ulong();
            if ($magic !== 0x5f0f3cf5) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Invalid head table magic ' . $magic);
            }
            $this->skip(2);
        } else {
            $this->skip(18);
        ...
        if ($glyphDataFormat != 0) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown glyph data format %s', $glyphDataFormat));
        }
        // hhea metrics table
        ...
        if ($debug) {
            ...
            if ($ver_maj < 1 || $ver_maj > 4) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown post table version %s', $ver_maj));
            }
        } else {
            $this->skip(4);
        }
        ...
        if ($debug) {
            $ver_maj = $this->read_ushort();
            if ($ver_maj != 1) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown hhea table version %s', $ver_maj));
            }
            $this->skip(28);
        } else {
            $this->skip(32);
        ...
        if ($metricDataFormat != 0) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown horizontal metric data format "%s"', $metricDataFormat));
        }
        $numberOfHMetrics = $this->read_ushort();
        if ($numberOfHMetrics == 0) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Error loading font: Number of horizontal metrics is 0');
        }
        ...
        if ($debug) {
            $ver_maj = $this->read_ushort();
            if ($ver_maj != 1) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Error loading font: Unknown maxp table version %s', $ver_maj));
            }
        } else {
            $this->skip(4);
        }
        ...
        if (!$unicode_cmap_offset) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $this->filename));
        }
        $sipset = \false;
        $smpset = \false;
        $this->rtlPUAstr = '';
                ...
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff && $bctr < 0x2ceb0) {
                        if (!$BMPonly) {
                            ...
                            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('The font "%s" does not have enough space to map all (unmapped) included glyphs into Private Use Area U+E000-U+F8FF', $names[1]));
                        }
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
    ...
    function _getGDEFtables()
    {
        ...
        if (isset($this->tables["GDEF"])) {
            ...
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Unable to set font "%s" to use OTL as it does not include OTL tables (or at least not a GDEF table).', $this->filename));
        }
        $GSUB_offset = 0;
        $GPOS_offset = 0;
        $GSUB_length = 0;
                ...
                */
                ...
                if ($Lookup[$i]['Type'] == 1) {
                    ...
                    if ($SubstFormat == 1) {
                        ...
                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Lookup[$i]['Type'] . ", Format " . $SubstFormat . " not supported (ttfontsuni.php).");
                    }
                    ...
                    throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Lookup Type "%s" not supported.', $Lookup[$i]['Type']));
                }
            }
        }
            ...
            for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
                ...
                if ($Lookup[$i]['Type'] == 1) {
                    ...
                    if ($SubstFormat == 1) {
                        ...
                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Lookup Type 5, SubstFormat 3 not tested. Please report this with the name of font used - " . $this->fontkey);
                    }
                } elseif ($Lookup[$i]['Type'] == 6) {
                    // Format 1: Simple Chaining Context Glyph Substitution  p255
                    if ($SubstFormat == 1) {
    ...
    function _getGSUBignoreString($flag, $MarkFilteringSet)
    {
        ...
        // Flag & 0x0010 = UseMarkFilteringSet
        if ($flag & 0x10) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException("This font " . $this->fontkey . " contains MarkGlyphSets - Not tested yet");
    ...
    */
    ...
    function makeSubset($file, &$subset, $TTCfontID = 0, $debug = \false, $useOTL = \false)
    {
        ...
        if (!$unicode_cmap_offset) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have Unicode cmap (platform 3, encoding 1, format 4, or platform 0 [any encoding] format 4)', $this->filename));
        }
        $glyphToChar = [];
        $charToGlyph = [];
        $this->getCMAP4($unicode_cmap_offset, $glyphToChar, $charToGlyph);
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    // Avoid overwriting a glyph already mapped in PUA
                    if ($bctr > 0xf8ff) {
                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException($file . " : WARNING - Font cannot map all included glyphs into Private Use Area U+E000 - U+F8FF; cannot use useOTL on this font");
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
                    $bctr++;
        ...
        foreach ($subsetglyphs as $originalGlyphIdx => $uni) {
            ...
            if ($glyphLen > 2 && $up[1] & 1 << 15) {
                ...
                $pos_in_glyph = 10;
                $flags = \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::MORE;
                $nComponentElements = 0;
                while ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::MORE) {
                    ...
                    $pos_in_glyph += 4;
                    if ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::WORDS) {
                        $pos_in_glyph += 4;
                    } else {
                        $pos_in_glyph += 2;
                    }
                    if ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::SCALE) {
                        $pos_in_glyph += 2;
                    } elseif ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::XYSCALE) {
                        $pos_in_glyph += 4;
                    } elseif ($flags & \GFPDF_Vendor\Mpdf\Fonts\GlyphOperator::TWOBYTWO) {
                        $pos_in_glyph += 8;
                    }
                }
    ...
    function makeSubsetSIP($file, &$subset, $TTCfontID = 0, $debug = \false, $useOTL = 0)
    {
        ...
        if ($TTCfontID > 0) {
            ...
            if (!\in_array($version, [0x10000, 0x20000])) {
                throw new \GFPDF_Vendor\Mpdf\Exception\FontException("ERROR - Error parsing TrueType Collection: version=" . $version . " - " . $file);
            }
            $this->numTTCFonts = $this->read_ulong();
            for ($i = 1; $i <= $this->numTTCFonts; $i++) {
                $this->TTCFonts[$i]['offset'] = $this->read_ulong();
        ...
        if (!$unicode_cmap_offset) {
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException(\sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $file));
        }
        // Format 12 CMAP does characters above Unicode BMP i.e. some HKCS characters U+20000 and above
        if ($format == 12) {
            $this->maxUniChar = 0;
    ...
    function getLOCA($indexToLocFormat, $numGlyphs)
    {
        ...
        if ($indexToLocFormat == 0) {
            ...
            throw new \GFPDF_Vendor\Mpdf\Exception\FontException('Unknown location table format ' . $indexToLocFormat);
        }
    }
    /**
     * CMAP Format 4
        ...
        if ($useOTL) {
            ...
            for ($gid = 1; $gid < $numGlyphs; $gid++) {
                if (!isset($glyphToChar[$gid])) {
                    ...
                    if ($bctr > 0xf8ff) {
                        throw new \GFPDF_Vendor\Mpdf\Exception\FontException("Problem. Trying to repackage TF file; not enough space for unmapped glyphs");
                    }
                    $glyphToChar[$gid][] = $bctr;
                    $charToGlyph[$bctr] = $gid;
                    $bctr++;
