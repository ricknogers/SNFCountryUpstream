...
namespace GFPDF_Vendor\Mpdf\Pdf;
...
class Protection
{
    ...
    public function __construct(\GFPDF_Vendor\Mpdf\Pdf\Protection\UniqidGenerator $uniqidGenerator)
    {
        if (!\function_exists('random_int') || !\function_exists('random_bytes')) {
            throw new \GFPDF_Vendor\Mpdf\MpdfException('Unable to set PDF file protection, CSPRNG Functions are not available. ' . 'Use paragonie/random_compat polyfill or upgrade to PHP 7.');
        }
        $this->uniqidGenerator = $uniqidGenerator;
        $this->lastRc4Key = '';
        $this->padding = "(¿N^NuŠAd\0NVÿú\1\10" . "..\0¶Ðh>€/\f©þdSiz";
    ...
    public function setProtection($permissions = [], $user_pass = '', $owner_pass = null, $length = 40)
    {
        ...
        if ($length === 128) {
            ...
            throw new \GFPDF_Vendor\Mpdf\MpdfException('PDF protection only allows lenghts of 40 or 128');
        }
        if ($owner_pass === null) {
    ...
    private function getProtectionBitsFromOptions($permissions)
    {
        ...
        foreach ($permissions as $permission) {
            if (!isset($this->options[$permission])) {
                throw new \GFPDF_Vendor\Mpdf\MpdfException(\sprintf('Invalid permission type "%s"', $permission));
            }
