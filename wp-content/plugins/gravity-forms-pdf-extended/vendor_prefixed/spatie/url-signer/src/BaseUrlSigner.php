...
abstract class BaseUrlSigner implements \GFPDF_Vendor\Spatie\UrlSigner\UrlSigner
{
     ...
     */
    protected function signUrl(\Psr\Http\Message\UriInterface $url, $expiration, $signature)
    {
     ...
     * @return UriInterface
     */
    protected function getIntendedUrl(\Psr\Http\Message\UriInterface $url)
    {
     ...
     * @return bool
     */
    protected function hasValidSignature(\Psr\Http\Message\UriInterface $url)
    {
        $query = \GFPDF_Vendor\League\Uri\QueryString::extract($url->getQuery());
        $expiration = $query[$this->expiresParameter];
