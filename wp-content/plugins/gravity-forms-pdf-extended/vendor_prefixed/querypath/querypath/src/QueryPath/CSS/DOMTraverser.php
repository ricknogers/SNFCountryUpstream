...
use GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util;
use GFPDF_Vendor\QueryPath\CSS\DOMTraverser\PseudoClass;
use GFPDF_Vendor\QueryPath\CSS\DOMTraverser\PseudoElement;
/**
 ...
 */
class DOMTraverser implements \GFPDF_Vendor\QueryPath\CSS\Traverser
{
    ...
    {
        $this->psHandler = new \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\PseudoClass();
    ...
    {
        // Setup
        $handler = new \GFPDF_Vendor\QueryPath\CSS\Selector();
        $parser = new \GFPDF_Vendor\QueryPath\CSS\Parser($selector, $handler);
        $parser->parse();
    ...
    public function combine($node, $selectors, $index)
    {
        ...
        switch ($selector->combinator) {
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::adjacent:
                return $this->combineAdjacent($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::sibling:
                return $this->combineSibling($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::directDescendant:
                return $this->combineDirectDescendant($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::anyDescendant:
                return $this->combineAnyDescendant($node, $selectors, $index);
            case \GFPDF_Vendor\QueryPath\CSS\SimpleSelector::anotherSelector:
    ...
    protected function matchAttributes($node, $attributes)
    {
        ...
        foreach ($attributes as $attr) {
            ...
            if (isset($attr['ns']) && $attr['ns'] != '*') {
                ...
                $matches = \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttributeNS($node, $attr['name'], $nsuri, $val, $attr['op']);
                ...
                foreach ($node->attributes as $attrNode) {
                    if ($attrNode->localName == $name) {
                        ...
                        $matches = \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttributeNS($node, $name, $nsuri, $val, $attr['op']);
                    }
                }
            } else {
                $matches = \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, $attr['name'], $val, $attr['op']);
            }
            if (!$matches) {
                return FALSE;
            }
    ...
    protected function matchPseudoElements($node, $pseudoElements)
    {
        ...
        foreach ($pseudoElements as $pse) {
            switch ($pse) {
                ...
                case 'selection':
                    throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException("::{$name} is not implemented.");
            }
        }
    }
