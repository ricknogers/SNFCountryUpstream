 ...
 */
class QueryPathEventHandler implements \GFPDF_Vendor\QueryPath\CSS\EventHandler, \GFPDF_Vendor\QueryPath\CSS\Traverser
{
    protected $dom = NULL;
    ...
    public function __construct($dom)
    {
        ...
        if (\is_array($dom) || $dom instanceof \SplObjectStorage) {
            ...
            throw new \GFPDF_Vendor\QueryPath\Exception("Unhandled type: " . \get_class($dom));
        }
        $this->matches = $matches;
    }
    /**
    ...
    {
        $parser = new \GFPDF_Vendor\QueryPath\CSS\Parser($filter, $this);
        $parser->parse();
        return $this;
    }
    /**
    ...
    public function attribute($name, $value = NULL, $operation = \GFPDF_Vendor\QueryPath\CSS\EventHandler::isExactly)
    {
        $found = new \SplObjectStorage();
        $matches = $this->candidateList();
        foreach ($matches as $item) {
    ...
    public function attributeNS($lname, $ns, $value = NULL, $operation = \GFPDF_Vendor\QueryPath\CSS\EventHandler::isExactly)
    {
        $matches = $this->candidateList();
        $found = new \SplObjectStorage();
        if (\count($matches) == 0) {
    ...
    public function pseudoClass($name, $value = NULL)
    {
        ...
        switch ($name) {
            ...
            case 'indeterminate':
                ...
                throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException(":indeterminate is not implemented.");
                break;
            case 'lang':
                // No value = exception.
                if (!isset($value)) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException("No handler for lang pseudoclass without value.");
                }
                $this->lang($value);
                break;
            case 'link':
            ...
            case 'not':
                if (empty($value)) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\ParseException(":not() requires a value.");
                }
                $this->not($value);
                break;
    ...
    */
    ...
    protected function not($filter)
    {
        ...
        $found = new \SplObjectStorage();
        foreach ($matches as $item) {
            $handler = new \GFPDF_Vendor\QueryPath\CSS\QueryPathEventHandler($item);
    ...
    protected function attrValMatches($needle, $haystack, $operation)
    {
        ...
        switch ($operation) {
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::isExactly:
                return $needle == $haystack;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsWithSpace:
                return \in_array($needle, \explode(' ', $haystack));
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsWithHyphen:
                return \in_array($needle, \explode('-', $haystack));
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsInString:
                return \strpos($haystack, $needle) !== FALSE;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::beginsWith:
                return \strpos($haystack, $needle) === 0;
            case \GFPDF_Vendor\QueryPath\CSS\EventHandler::endsWith:
    ...
    public function pseudoElement($name)
    {
        ...
        switch ($name) {
            ...
            case 'selection':
                ...
                throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException("The {$name} pseudo-element is not implemented.");
                break;
        }
        $this->findAnyElement = FALSE;
    }
