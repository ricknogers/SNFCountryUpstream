 ...
 */
namespace GFPDF_Vendor\QueryPath\CSS\DOMTraverser;

use GFPDF_Vendor\QueryPath\CSS\NotImplementedException;
use GFPDF_Vendor\QueryPath\CSS\EventHandler;
/**
...
class PseudoClass
{
    ...
    public function elementMatches($pseudoclass, $node, $scope, $value = NULL)
    {
        ...
        switch ($name) {
            ...
            case 'lang':
                // No value = exception.
                if (!isset($value)) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\NotImplementedException(":lang() requires a value.");
                }
                return $this->lang($node, $value);
            case 'any-link':
                return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'href') || \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'src') || \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'link');
            case 'link':
                return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'href');
            case 'local-link':
            ...
            case 'disabled':
            case 'checked':
                return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, $name);
            case 'text':
            case 'radio':
            case 'checkbox':
            case 'file':
            ...
            case 'reset':
            case 'button':
                return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'type', $name);
            case 'header':
            ...
            case 'not':
                if (empty($value)) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\ParseException(":not() requires a value.");
                }
            ...
            default:
                throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("Unknown Pseudo-Class: " . $name);
        }
    ...
    protected function lang($node, $value)
    {
        ...
        $operator = \strpos($value, '-') !== FALSE ? \GFPDF_Vendor\QueryPath\CSS\EventHandler::isExactly : \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsWithHyphen;
        $match = TRUE;
        foreach ($node->attributes as $attrNode) {
            if ($attrNode->localName == 'lang') {
                if ($attrNode->nodeName == $attrNode->localName) {
                    ...
                    return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttribute($node, 'lang', $value, $operator);
                } else {
                    $nsuri = $attrNode->namespaceURI;
                    // fprintf(STDOUT, "%s in NS %s\n", $attrNode->name, $nsuri);
                    return \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::matchesAttributeNS($node, 'lang', $nsuri, $value, $operator);
                }
            }
        }
        return FALSE;
    ...
    protected function contains($node, $value)
    {
        ...
        $value = \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::removeQuotes($value);
    ...
    {
        $splos = new \GFPDF_Vendor\SPLObjectStorage();
        $splos->attach($node);
        $traverser = new \GFPDF_Vendor\QueryPath\CSS\DOMTraverser($splos, TRUE);
    ...
    {
        list($groupSize, $elementInGroup) = \GFPDF_Vendor\QueryPath\CSS\DOMTraverser\Util::parseAnB($value);
