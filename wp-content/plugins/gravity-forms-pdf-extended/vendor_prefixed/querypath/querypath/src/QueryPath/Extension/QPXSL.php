 ...
 */
class QPXSL implements \GFPDF_Vendor\QueryPath\Extension
{
    protected $src = NULL;
    ...
    {
        if (!$style instanceof \GFPDF_Vendor\QueryPath\Extension\QueryPath) {
            $style = \GFPDF_Vendor\QueryPath::with($style);
        }
        $sourceDoc = $this->src->top()->get(0)->ownerDocument;
        $styleDoc = $style->get(0)->ownerDocument;
        $processor = new \XSLTProcessor();
        $processor->importStylesheet($styleDoc);
        return \GFPDF_Vendor\QueryPath::with($processor->transformToDoc($sourceDoc));
    }
}
