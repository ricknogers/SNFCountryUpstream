...
class Parser
{
     ...
     */
    public function __construct($string, \GFPDF_Vendor\QueryPath\CSS\EventHandler $handler)
    {
        $this->originalString = $string;
        $is = new \GFPDF_Vendor\QueryPath\CSS\InputStream($string);
        $this->scanner = new \GFPDF_Vendor\QueryPath\CSS\Scanner($is);
    ...
    public function parse()
    {
        ...
        while ($this->scanner->token !== FALSE) {
            ...
            if ($this->scanner->token !== FALSE && $finalPosition == $position) {
                ...
                // pattern.
                throw new \GFPDF_Vendor\QueryPath\CSS\ParseException('CSS selector is not well formed.');
            }
        }
    }
    /**
    ...
    private function consumeWhitespace()
    {
        ...
        $white = 0;
        while ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::white) {
            $this->scanner->nextToken();
            ++$white;
        }
        return $white;
    ...
    private function combinator()
    {
        ...
        if ($t == \GFPDF_Vendor\QueryPath\CSS\Token::rangle) {
            $this->handler->directDescendant();
            $this->scanner->nextToken();
            $inCombinator = TRUE;
            //$this->simpleSelectors();
        } elseif ($t == \GFPDF_Vendor\QueryPath\CSS\Token::plus) {
            $this->handler->adjacent();
            $this->scanner->nextToken();
            $inCombinator = TRUE;
            //$this->simpleSelectors();
        } elseif ($t == \GFPDF_Vendor\QueryPath\CSS\Token::comma) {
            $this->handler->anotherSelector();
            $this->scanner->nextToken();
            $inCombinator = TRUE;
            //$this->scanner->selectors();
        } elseif ($t == \GFPDF_Vendor\QueryPath\CSS\Token::tilde) {
        ...
        if ($inCombinator) {
            $white = 0;
            if ($this->DEBUG) {
                print "COMBINATOR: " . \GFPDF_Vendor\QueryPath\CSS\Token::name($t) . "\n";
            }
            $this->consumeWhitespace();
            if ($this->isCombinator($this->scanner->token)) {
                throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("Illegal combinator: Cannot have two combinators in sequence.");
            }
        } elseif ($white > 0) {
            if ($this->DEBUG) {
    ...
    {
        $combinators = array(\GFPDF_Vendor\QueryPath\CSS\Token::plus, \GFPDF_Vendor\QueryPath\CSS\Token::rangle, \GFPDF_Vendor\QueryPath\CSS\Token::comma, \GFPDF_Vendor\QueryPath\CSS\Token::tilde);
    ...
    private function elementID()
    {
        ...
        if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::octo) {
            $this->scanner->nextToken();
            if ($this->scanner->token !== \GFPDF_Vendor\QueryPath\CSS\Token::char) {
                throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("Expected string after #");
            }
    ...
    private function elementClass()
    {
        ...
        if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::dot) {
            $this->scanner->nextToken();
            $this->consumeWhitespace();
            // We're very fault tolerent. This should prob through error.
    ...
    private function pseudoClass($restricted = FALSE)
    {
        ...
        if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::colon) {
            // Check for CSS 3 pseudo element:
            $isPseudoElement = FALSE;
            if ($this->scanner->nextToken() === \GFPDF_Vendor\QueryPath\CSS\Token::colon) {
                $isPseudoElement = TRUE;
                $this->scanner->nextToken();
            }
            $name = $this->scanner->getNameString();
            if ($restricted && $name == 'not') {
                throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("The 'not' pseudo-class is illegal in this context.");
            }
            $value = NULL;
            if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::lparen) {
                if ($isPseudoElement) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("Illegal left paren. Pseudo-Element cannot have arguments.");
                }
            ...
            if ($isPseudoElement) {
                if ($restricted) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("Pseudo-Elements are illegal in this context.");
                }
                ...
                if ($this->scanner->token !== FALSE && $this->scanner->token !== \GFPDF_Vendor\QueryPath\CSS\Token::comma) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\ParseException("A Pseudo-Element must be the last item in a selector.");
                }
            } else {
    ...
    private function elementName()
    {
        ...
        if ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::pipe) {
            // We have '|name', which is equiv to 'name'
            $this->scanner->nextToken();
            $this->consumeWhitespace();
            $elementName = $this->scanner->getNameString();
            $this->handler->element($elementName);
        } elseif ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::char) {
            $elementName = $this->scanner->getNameString();
            if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::pipe) {
                // Get ns|name
                ...
                $this->scanner->nextToken();
                $this->consumeWhitespace();
                if ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::star) {
                    // We have ns|*
                    $this->handler->anyElementInNS($elementNS);
                    $this->scanner->nextToken();
                } elseif ($this->scanner->token !== \GFPDF_Vendor\QueryPath\CSS\Token::char) {
                    $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::char, $this->scanner->token);
                } else {
                    $elementName = $this->scanner->getNameString();
                    // We have ns|name
    ...
    {
        if ($this->scanner->token == \GFPDF_Vendor\QueryPath\CSS\Token::lsquare) {
            $attrVal = $op = $ns = NULL;
            $this->scanner->nextToken();
            $this->consumeWhitespace();
            if ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::at) {
                if ($this->strict) {
                    throw new \GFPDF_Vendor\QueryPath\CSS\ParseException('The @ is illegal in attributes.');
                } else {
                    $this->scanner->nextToken();
                    $this->consumeWhitespace();
                }
            ...
            if ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::pipe && $this->scanner->peek() !== '=') {
            ...
            switch ($this->scanner->token) {
                case \GFPDF_Vendor\QueryPath\CSS\Token::eq:
                    $this->consumeWhitespace();
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::isExactly;
                    break;
                case \GFPDF_Vendor\QueryPath\CSS\Token::tilde:
                    if ($this->scanner->nextToken() !== \GFPDF_Vendor\QueryPath\CSS\Token::eq) {
                        $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::eq, $this->scanner->token);
                    }
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsWithSpace;
                    break;
                case \GFPDF_Vendor\QueryPath\CSS\Token::pipe:
                    if ($this->scanner->nextToken() !== \GFPDF_Vendor\QueryPath\CSS\Token::eq) {
                        $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::eq, $this->scanner->token);
                    }
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsWithHyphen;
                    break;
                case \GFPDF_Vendor\QueryPath\CSS\Token::star:
                    if ($this->scanner->nextToken() !== \GFPDF_Vendor\QueryPath\CSS\Token::eq) {
                        $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::eq, $this->scanner->token);
                    }
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::containsInString;
                    break;
                case \GFPDF_Vendor\QueryPath\CSS\Token::dollar:
                    if ($this->scanner->nextToken() !== \GFPDF_Vendor\QueryPath\CSS\Token::eq) {
                        $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::eq, $this->scanner->token);
                    }
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::endsWith;
                    break;
                case \GFPDF_Vendor\QueryPath\CSS\Token::carat:
                    if ($this->scanner->nextToken() !== \GFPDF_Vendor\QueryPath\CSS\Token::eq) {
                        $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::eq, $this->scanner->token);
                    }
                    $op = \GFPDF_Vendor\QueryPath\CSS\EventHandler::beginsWith;
                    break;
            }
            if (isset($op)) {
                // Consume '=' and go on.
                ...
                if ($this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::quote || $this->scanner->token === \GFPDF_Vendor\QueryPath\CSS\Token::squote) {
            ...
            $this->consumeWhitespace();
            if ($this->scanner->token != \GFPDF_Vendor\QueryPath\CSS\Token::rsquare) {
                $this->throwError(\GFPDF_Vendor\QueryPath\CSS\Token::rsquare, $this->scanner->token);
            }
            if (isset($ns)) {
    ...
    {
        $filter = \sprintf('Expected %s, got %s', \GFPDF_Vendor\QueryPath\CSS\Token::name($expected), \GFPDF_Vendor\QueryPath\CSS\Token::name($got));
        throw new \GFPDF_Vendor\QueryPath\CSS\ParseException($filter);
    }
}
