 ...
 */
class QPXML implements \GFPDF_Vendor\QueryPath\Extension
{
    protected $qp;
    ...
    public function schema($file)
    {
        ...
        if (!$doc->schemaValidate($file)) {
            throw new \GFPDF_Vendor\QueryPath\Exception('Document did not validate against the schema.');
        }
    }
    /**
        ...
        if (isset($text)) {
            foreach ($this->qp->get() as $element) {
                if ($nsUri === null && \strpos($text, ':') !== \false) {
                    ...
                    if ($nsUri === null) {
                        throw new \GFPDF_Vendor\QueryPath\Exception("Undefined namespace for: " . $text);
                    }
                }
                $node = null;
                if ($nsUri !== null) {
                ...
                return \GFPDF_Vendor\QueryPath::with($node);
            }
        }
        return;
    }
    ...
    public function appendElement($text)
    {
        if (isset($text)) {
            foreach ($this->qp->get() as $element) {
                ...
                \GFPDF_Vendor\QueryPath::with($element)->append($node);
            }
        }
        return $this->qp;
    }
