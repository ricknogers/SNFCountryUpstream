...
final class ExactMetrics_Report_Dimensions extends ExactMetrics_Report {
	...
	public function prepare_report_data( $data ) {
		...
		if ( ! empty( $data['data'] ) ) {
			...
			foreach ( $data['data']['dimensions'] as $key => $dimension_data ) {
				$data['data']['galinks'][ $key ] = 'https://analytics.google.com/analytics/web/#/report/visitors-custom-variables/' . ExactMetrics()->auth->get_referral_url() . $this->get_ga_report_range( $data['data'] ) . rawurlencode( '&explorer-segmentExplorer.segmentId=analytics.customVarName' . $key . '&explorer-table.plotKeys=[]/' );
			}
		}
	...
	public function add_dimensions_settings_link( $data ) {
		if ( current_user_can( 'exactmetrics_save_settings' ) ) {
			$data['data']['footer'] = sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=exactmetrics_settings#/conversions' ), esc_html__( 'View dimensions settings', 'exactmetrics-premium' ) );
		}
