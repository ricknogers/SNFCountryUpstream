...
final class ExactMetrics_License_Actions {
	...
	public function maybe_verify_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || strlen( $_POST['exactmetrics-license-key'] ) < 10 || empty( $_POST['exactmetrics-verify-submit'] ) ) {
			return;
		}

		if ( ! wp_verify_nonce( $_POST['exactmetrics-key-nonce'], 'exactmetrics-key-nonce' ) ) {
			return;
		}
	...
	public function verify_key( $key = '' ) {
		...
		if ( empty( $key ) ) {
			$key = $_POST['exactmetrics-license-key'];
		}
	...
	public function maybe_deactivate_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || empty( $_POST['exactmetrics-deactivate-submit'] ) ) {
			return;
		}
	...
	public function deactivate_key() {
		...
		$deactivate = $this->perform_remote_request( 'deactivate-key', array( 'tgm-updater-key' => $_POST['exactmetrics-license-key'] ) );
	...
	public function maybe_refresh_key() {
		...
		if ( empty( $_POST['exactmetrics-license-key'] ) || empty( $_POST['exactmetrics-refresh-submit'] ) ) {
			return;
		}
